...
//             userId: req.customer ? req.customer._id : null,
//             guestInfo: req.customer ? null : req.body.guestInfo,
//             items: orderItems,
...
exports.cancelOrder = async (req, res) => {
    try {
        ...
        // Allow cancellation only for the customer's own order
        if (order.customerId && order.customerId.toString() !== req.customer._id.toString()) {
...
exports.getAllOrders = async (req, res) => {
    try {
        ...
        const orders = await Order.find(filter)
            .sort({ createdAt: -1 })
            .populate("customerId", "name email"); // لو عايزة بيانات اليوزر
